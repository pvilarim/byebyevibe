...
1. **If no change name provided, prompt for selection**
   ...
   Run `openspec list --json` to get available changes. Ask the user in chat (plain text) to select one.
...
2. **Check artifact completion status**
   ...
   **If any artifacts are not `done`:**
   ...
   - Ask the user in chat (plain text) to confirm they want to proceed
...
4b. **Consolidated closing assessment (§12.10)**
   ...
   1. **Reusable pattern** — did the change establish a procedure or template used more than once or likely to recur? Evidence: the change's own `tasks.md`/`design.md`. Promotion targets: a reusable skill (`.cursor/skills/<domain>-pattern/SKILL.md`) or a note in `openspec/project.md` Cross-references.
   ...
   **Only when ≥1 item has a positive signal**, present ONE consolidated question in chat (plain text), listing only the positively-signaled items plus a "none of these / proceed" option.
