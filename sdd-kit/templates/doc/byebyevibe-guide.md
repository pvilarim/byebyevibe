...
# Verify BEFORE extracting. Explicit comparison rather than `sha256sum -c`: macOS has no
# GNU sha256sum on PATH by default, and the -c mode is not portable across the two tools.
EXPECTED=$(cut -d' ' -f1 < "$TMPDIR/byebyevibe-kit.tar.gz.sha256")
ACTUAL=$(sha256sum "$TMPDIR/byebyevibe-kit.tar.gz" 2>/dev/null || shasum -a 256 "$TMPDIR/byebyevibe-kit.tar.gz")
ACTUAL=${ACTUAL%% *}
if [ -n "$EXPECTED" ] && [ "$EXPECTED" = "$ACTUAL" ]; then
  ...
  echo "Kit acquired. Next: bash scripts/bootstrap-sdd.sh --profile <PROFILE>"
else
  echo "CHECKSUM MISMATCH — expected '$EXPECTED', got '$ACTUAL'. NOTHING was extracted." >&2
fi
rm -rf "$TMPDIR"
```

Verification precedes extraction deliberately: a checksum checked after the archive is already unpacked verifies nothing worth having. The recipe enforces that by construction — extraction is inside the match branch, and a mismatch says so loudly and writes nothing to the repository. An empty `EXPECTED` (sidecar download silently failed) is treated as a mismatch, never as a pass.
...
# Explicit comparison, not `sha256sum -c` — portable to macOS, where GNU sha256sum is
# absent by default and only `shasum -a 256` is guaranteed.
EXPECTED=$(cut -d' ' -f1 < "byebyevibe-kit-${VERSION}.tar.gz.sha256")
ACTUAL=$(sha256sum "byebyevibe-kit-${VERSION}.tar.gz" 2>/dev/null || shasum -a 256 "byebyevibe-kit-${VERSION}.tar.gz")
ACTUAL=${ACTUAL%% *}
if [ -n "$EXPECTED" ] && [ "$EXPECTED" = "$ACTUAL" ]; then
  echo "MATCH — regenerated archive is byte-identical to the published one"
else
  echo "MISMATCH — expected '$EXPECTED', got '$ACTUAL' (check git version first, see below)" >&2
fi
```
