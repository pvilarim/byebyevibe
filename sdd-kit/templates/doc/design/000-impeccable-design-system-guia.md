...
> - **Next step:** integrate the pipeline (Open Design → Pencil/Figma → shadcn → Impeccable) into the canonical guide `doc/byebyevibe-guide.md` and the install kit `sdd-kit/`, for distribution to any repository using the SDD stack (OpenSpec + GitNexus + Graphify).
...
| `doc/byebyevibe-guide.md` | Canonical SDD install guide — **future destination for this pipeline** |
