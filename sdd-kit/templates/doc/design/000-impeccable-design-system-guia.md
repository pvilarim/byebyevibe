...
> - **Next step:** integrate the pipeline (Open Design → Pencil/Figma → shadcn → Impeccable) into the canonical guide `doc/sistema-sdd-pedro.md` and the install kit `sdd-kit/`, for distribution to any repository using the SDD stack (OpenSpec + GitNexus + Graphify).
...
| `openspec/project.md` | Project constitution (stack, profiles) |
| `doc/sistema-sdd-pedro.md` | Canonical SDD install guide — **future destination for this pipeline** |
