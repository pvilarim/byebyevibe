...
The hub also ships `.github/workflows/automated-pr-review.yml` (issue #349). It is **not** a C1/C2 payload: `install.sh` / `upgrade.sh` do not copy it, and no MANIFEST entry enables a credentialed paid review. Operators who want the same pipeline in a consumer repo must copy it explicitly after a separately reviewed change. See guide §2.19.
