...
Each precondition MUST be non-vacuous: a check that produced no result MUST report failure, not success. A precondition whose helper process failed to run, or ran and compared nothing it had selected for comparison, MUST NOT be recorded as satisfied. This is not a theoretical safeguard — the readiness check has passed on a host where its own subprocess produced no output, so the operator was told the repository was release-ready by a check that had examined nothing.

The distinction that keeps this compatible with the readiness script's other consumers (the CI gate runs it in consumer repositories, where hub-only subjects are legitimately absent): a **subject absent in context** remains a recorded skip, as the CI-gates capability already specifies; a **helper that failed to run or to produce its result** is never a skip and never a pass. The discriminator is whether the check had something applicable to examine and its own machinery failed, not whether output happened to be empty.
