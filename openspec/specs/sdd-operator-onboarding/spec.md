# sdd-operator-onboarding Specification

## Purpose

Normative requirements for ByeByeVibe day-1 operate onboarding: kit-owned `/opsx:help` (skill + command mirrors), canonical English `doc/sdd-operator-day1.md`, complementary Onboard vs Help framing, phase tutorial spine, clickable control-plane map, explore prompt craft, per-phase confidence prompts, install tip naming both slash commands, AGENTS Commands row, optional soft checklist pointer, and evaluation stub — without always-on rules or patching upstream `openspec-onboard`.
