# sdd-probity-module Specification

## Purpose

Normative requirements for the optional **Probity module** (G2) distributed via `sdd-kit/`, separate from core SDD install (C1). Covers `install-probity-module.sh`, pinned `@nizos/probity@1.10.0`, template `probity.config.ts` with `enforceTdd()`, pilot-before-MANIFEST-promotion, six-point contract registration, and documentation supersession of TDD Guard by Probity (2026-07).
