# sdd-tooling-guidance Specification

## Purpose

Normative requirements for how the agent reaches external tools and suggests missing integrations: the resolution cascade (override → CLI → MCP → suggest → manual), CLI-first default with per-tool MCP exceptions, session-scoped operator override, security-hardened offer-only suggestion message under the shared anti-noise cap, durable refusals, conversational signal catalog behind the static gap-check, archive-time manual-work question, per-tool install documentation in `doc/tooling-install.md`, dual Claude Code + Cursor delivery with documented asymmetry, and v2 scope recorded as deferred non-goals.
