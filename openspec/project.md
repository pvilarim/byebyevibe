...
- We do not run the SDD pipeline (explore→propose→apply→archive) from a code-first orchestration runtime — phase state lives in durable, reviewable artifacts (`tasks.md`, `openspec/changes/`), never in a volatile process; Deer Workflow evaluated and discarded as kit payload, deferred as a bounded fan-out runner (see `doc/avaliacoes/2026-08-13-deer-workflow.md`)
...
- We do not add a third codebase graph alongside GitNexus (code) and Graphify (concepts) — NanoNets Graft occupies both layers plus always-on `AGENTS.md` / Cursor-rule wiring; discarded as kit payload, deferred as a freshness / statusline / SWE-bench pattern source (see `doc/avaliacoes/2026-08-17-nanonets-graft.md`)
