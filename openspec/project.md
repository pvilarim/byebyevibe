...
- **SDD install guide:** `doc/byebyevibe-guide.md` **v1.8.0** — install §2 + `sdd-kit/install.sh`; upgrade §2.9 + `sdd-kit/upgrade.sh`; UI module §2.11 + `sdd-kit/install-ui-module.sh`; CI gates §2.12; Probity (G2) §2.16 + `sdd-kit/install-probity-module.sh`; SDD metrics (G4) §2.17 + `scripts/sdd-metrics.sh` (playbook + cadence `--check-cadence`)
- **Install kit:** `sdd-kit/` (MANIFEST v1.8.0) — versioned payloads for C1/C2/C1-UI/G2/G4 + workflow `sdd-gates`
