...
- [ ] 1.2 Mirror the same rewrite in `.claude/commands/opsx/archive.md`
...
- [ ] 1.3 Mirror the rewrite in `.cursor/skills/openspec-archive-change/SKILL.md` with the Cursor asymmetry (plain-text chat question instead of `AskUserQuestion`; no Claude Code harness tool names)
...
- [ ] 1.4 Mirror the rewrite in `.cursor/commands/opsx-archive.md` (same Cursor asymmetry)
...
- [ ] 2.1 Update the "At archive time, the workflow asks…" sentence in the installed copies `.claude/skills/sdd-skill-guidance/SKILL.md` and `.cursor/skills/sdd-skill-guidance/SKILL.md` to describe the agent-evaluated consolidated assessment (verdict always printed, prompt only on positive signal)
...
- [ ] 2.2 Apply the same sentence update to the kit templates `sdd-kit/templates/.claude/skills/sdd-skill-guidance/SKILL.md` and `sdd-kit/templates/.cursor/skills/sdd-skill-guidance/SKILL.md`
...
- [ ] 3.1 Regenerate manifest checksums after the template edits and verify the kit
  - **Pattern:** `sdd-kit/MANIFEST.yaml`
  - **Gate:** `bash sdd-kit/gen-manifest-checksums.sh && bash sdd-kit/verify.sh`

## 4. Validation

- [ ] 4.1 Validate all specs and deltas strictly
  - **Gate:** `npx -y @fission-ai/openspec@1.3.1 validate --all --strict`
- [ ] 4.2 Verify task pattern paths resolve
