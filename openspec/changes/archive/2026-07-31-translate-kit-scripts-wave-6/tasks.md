...
- [x] 1.1 Confirm glossary covers terms needed for this wave (`install kit`, `gate`, `wave`, `session`); append rows to `doc/i18n/GLOSSARY.md` only if a new SDD term is introduced (same PR)
...
- [x] 2.1 Rewrite `sdd-kit/templates/install-ui-module.sh` Portuguese embedded infra UI-module table chrome and any residual Portuguese comments/operator strings → glossary-canonical English; keep `--detect` / `--dry-run` / `--apply` / `--yes` / stack detection / Impeccable install control flow intact
...
- [x] 2.2 Regenerate kit MANIFEST checksums for the edited template source
...
- [x] 3.1 Run per-wave i18n verification on the exact template path
