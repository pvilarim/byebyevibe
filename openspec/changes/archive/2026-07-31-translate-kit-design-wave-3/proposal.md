...
Hub design waves 1–2 own `doc/design/002|003|004` and `000`. The pipeline reference `doc/design/001-pipeline-open-design-shadcn-impeccable.md` (~592 LOC) was explicitly deferred (over single-wave budget). This change covers lines **1–325** (kit mirror §1–§3.4, ~325 LOC) — within ≤350–400 LOC. Kit mirror follows hub EN for the same slice.

## What Changes

- Replace Portuguese prose with glossary-canonical English **in-place** in:
  - `sdd-kit/templates/doc/design/001-pipeline-open-design-shadcn-impeccable.md` (lines **1–325** only)
...
- `sdd-docs-language`: ADDED requirement — `sdd-kit/templates/doc/design/001-pipeline-open-design-shadcn-impeccable.md` slice lines 1–325 MUST be English after substitution.

## Impact

- Files modified: `sdd-kit/templates/doc/design/001-pipeline-open-design-shadcn-impeccable.md` (lines 1–325); `sdd-kit/MANIFEST.yaml` checksums only; optional `doc/i18n/GLOSSARY.md`
- Dependencies: soft apply preference — hub `translate-design-wave-3` apply+archive before kit apply; serialize vs other in-flight `sdd-kit/templates/` applies. Infra ✅
...
/opsx:apply translate-kit-design-wave-3

Change: openspec/changes/translate-kit-design-wave-3/
