...
- [x] 2.1 Rewrite `sdd-kit/README.md` residual Portuguese prose → glossary-canonical English at the same path; keep ByeByeVibe branding, scenario codes (`C1`/`C2`/`C2b`/`C3`/`C1-UI`/`G2`/`G4`), profile names, and all fenced `bash` blocks byte-stable
...
- [x] 2.2 Rewrite `sdd-kit/templates/AGENTS.core.md` Portuguese prose → English aligned with hub W1 section titles where mirrored; translate `[PREENCHER:…]` placeholders to `[FILL:…]`; freeze `<!-- SDD_KIT_COMMANDS_START -->` / `<!-- SDD_KIT_COMMANDS_END -->`, paths, `/opsx:*`, pins, and R1–R11 numbering
...
- [x] 2.3 Rewrite `sdd-kit/templates/AGENTS.commands.DOCS_SPECS.md` Portuguese table headers/cells → English; freeze all fenced/command column values (`npx openspec`, `graphify`, `bash scripts/…`)
...
- [x] 2.4 Rewrite `sdd-kit/templates/AGENTS.commands.APP.md` Portuguese table headers/cells → English; freeze command column values (`npm run dev`, `npx openspec`, `graphify`, session/metrics scripts)
...
- [x] 4.1 Run per-wave i18n verification on the exact W2 file list
...
- [x] 5.1 `graphify update .` if available (docs/templates touched); otherwise SKIP per infra
  ...
  - **Note:** SKIP (graphify CLI unavailable; local stub only — not committed)
