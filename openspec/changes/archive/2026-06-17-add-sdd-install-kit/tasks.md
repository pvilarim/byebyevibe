...
- [x] 1.1 Criar `sdd-kit/MANIFEST.yaml` v1.3.0 com lista completa de ficheiros, merge strategy e gates
...
- [x] 1.2 Criar `sdd-kit/README.md` (C1/C2/C2b/C3, comandos, perfis)
  - **Pattern:** `doc/sistema-sdd-pedro.md` §2.8
  - **Gate:** `grep -q 'C1' sdd-kit/README.md && grep -q 'C3' sdd-kit/README.md`

- [x] 1.3 Popular `sdd-kit/templates/` a partir do estado actual do piloto
...
- [x] 1.4 Criar `sdd-kit/templates/AGENTS.core.md` e fragments `AGENTS.commands.APP.md` / `AGENTS.commands.DOCS_SPECS.md`
...
- [x] 2.1 Implementar `sdd-kit/install.sh` (`--profile`, `--dry-run`, COPY/MERGE)
...
- [x] 2.2 Implementar `sdd-kit/upgrade.sh` (`--from`, `--to`, `--dry-run`, `--apply`)
...
- [x] 2.3 Implementar `sdd-kit/verify.sh`
  - **Pattern:** `scripts/verify-infra.sh`
  - **Gate:** `bash sdd-kit/verify.sh; test $? -eq 0 || test $? -eq 1`  # 0 se tudo OK; script existe e corre

- [x] 2.4 Actualizar `scripts/sdd-upgrade-diff.sh` para ler paths de `sdd-kit/MANIFEST.yaml`
  - **Pattern:** `scripts/sdd-upgrade-diff.sh`
  - **Gate:** `bash scripts/sdd-upgrade-diff.sh 2>&1 | grep -q '015-session-phases'`

- [x] 2.5 Actualizar `scripts/bootstrap-sdd.sh` para referenciar `sdd-kit/install.sh` pós-CLIs
  - **Pattern:** `scripts/bootstrap-sdd.sh`
  - **Gate:** `grep -q 'sdd-kit/install' scripts/bootstrap-sdd.sh`

## 3. Guia canónico v1.3.0

- [x] 3.1 Adicionar §1.6 *Organização do projecto e tipos de instalação* (4 camadas, C1–C3, perfis, hub vs APP)
...
- [x] 3.2 Actualizar tabela "Como usar este documento" e §2.0 prompt IA para `sdd-kit/install.sh`
  - **Pattern:** `doc/sistema-sdd-pedro.md` §2.0
  - **Gate:** `grep -q 'sdd-kit/install' doc/sistema-sdd-pedro.md`

- [x] 3.3 Actualizar §2.9.5 matriz e §12.9 staging — fonte = `sdd-kit/templates/`, não extração markdown
  - **Pattern:** `doc/sistema-sdd-pedro.md` §2.9.5
  - **Gate:** `grep -q 'sdd-kit/templates' doc/sistema-sdd-pedro.md`

- [x] 3.4 Bump versão guia para v1.3.0 + changelog §14 (install kit, §1.6, session coord no changelog)
...
- [x] 3.5 Deprecar em §12 blocos de scripts inteiros — substituir por ponteiro `sdd-kit/templates/`
  - **Pattern:** `doc/sistema-sdd-pedro.md` §12.6
  - **Gate:** `grep -q 'sdd-kit/templates' doc/sistema-sdd-pedro.md`

## 4. Infra, project e AGENTS

- [x] 4.1 Secção Install Kit em `openspec/infra.md`
  - **Pattern:** `openspec/infra.md` secção Session Coordination
  - **Gate:** `grep -q 'Install Kit' openspec/infra.md`

- [x] 4.2 Actualizar `openspec/project.md` Cross-references → guia v1.3.0 + `sdd-kit/`
  - **Pattern:** `openspec/project.md`
  - **Gate:** `grep -q 'v1.3.0' openspec/project.md && grep -q 'sdd-kit' openspec/project.md`

- [x] 4.3 Entrada `sdd-kit/` na tabela Contexto sob demanda de `AGENTS.md` (se ≤150 linhas)
  - **Pattern:** `AGENTS.md` linha infra.md
  - **Gate:** `test $(wc -l < AGENTS.md) -le 150 && grep -q 'sdd-kit' AGENTS.md`

- [x] 4.4 Actualizar `scripts/verify-infra.sh` para validar kit version / paths MANIFEST
  - **Pattern:** `scripts/verify-infra.sh`
  - **Gate:** `grep -q 'MANIFEST\|sdd-kit' scripts/verify-infra.sh`

## 5. Validação e fecho

- [x] 5.1 `bash scripts/verify-task-patterns.sh` nos tasks deste change
  - **Gate:** `bash scripts/verify-task-patterns.sh`

- [x] 5.2 `npx openspec validate add-sdd-install-kit` (se CLI disponível)
  - **Gate:** `npx openspec validate add-sdd-install-kit 2>/dev/null || test -f openspec/changes/add-sdd-install-kit/proposal.md`

- [x] 5.3 Teste integrado: `bash sdd-kit/verify.sh` no piloto pós-apply
  - **Gate:** `bash sdd-kit/verify.sh`

- [x] 5.4 Commit: `feat(sdd): install kit v1.3.0 (add-sdd-install-kit)`
