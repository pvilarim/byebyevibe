...
- [x] 1.1 Criar `.claude/skills/correctness-review/SKILL.md` com: frontmatter (name/description/license/metadata), secções Quando invocar (matriz A–E), Formato de saída (tags logic/edge/contract/race/silent, vereditos CORRECT/RISKY/ESCOPO INSUFICIENTE), Boundaries (nunca flaggar), Integração SDD e Comandos úteis
...
- [x] 1.2 Criar `.cursor/skills/correctness-review/SKILL.md` como cópia idêntica do item 1.1
...
- [x] 2.1 Actualizar secção "Reviews pós-implementação" em `AGENTS.md`: adicionar linha `correctness-review` com posição na pipeline (antes de `simplify-review`) e quando invocar/não invocar segundo a matriz A–E; actualizar a linha de ordem do pipeline
  - **Pattern:** `AGENTS.md`
...
- [x] 3.1 Adicionar linha de `correctness-review` na secção Skills de `openspec/infra.md` (path, fase review, estado ✅)
...
- [x] 4.1 Adicionar subsecção `correctness-review` em `doc/sistema-sdd-pedro.md` (posição lógica após `simplify-review`): quando acionar, como ler output (tags, vereditos), como não acionar, troubleshooting
...
- [x] 5.1 Actualizar `doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md`: G7 → "Adoptado — skill local criada, change `add-correctness-review-skill`" + referência a este change + condições de reavaliação (conversão para subagente, PR-Agent fase 2)
...
- [x] 6.1 Adicionar nota de instalação manual em `sdd-kit/README.md` (análogo ao `simplify-review`): path dos ficheiros de skill, que não há script automático nesta fase, e que o apply promove a spec para `openspec/specs/`
  - **Pattern:** `sdd-kit/README.md`
  - **Gate:** `grep -qi 'correctness-review' sdd-kit/README.md`

- [x] 6.2 Promover `openspec/changes/add-correctness-review-skill/specs/sdd-correctness-review/spec.md` para `openspec/specs/sdd-correctness-review/spec.md`
...
- [x] 7.1 Correr `scripts/verify-task-patterns.sh` sobre este `tasks.md`
  - **Pattern:** `scripts/verify-task-patterns.sh`
  - **Gate:** `bash scripts/verify-task-patterns.sh`

- [x] 7.2 Validar change com openspec CLI
...
- [x] 8.1 `graphify update .` + `npx gitnexus analyze --force` se disponíveis
