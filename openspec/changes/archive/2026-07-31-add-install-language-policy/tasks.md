...
- [x] 1.1 Update `sdd-kit/templates/AGENTS.core.md` Communication: replace `[Adapt: pt-BR]` with three-axis F7 block and `{{CHAT_LANG}}`, `{{DOCS_LANG}}`, `{{CODE_LANG}}` placeholders
...
- [x] 1.2 Add Language policy snippet to guide §12.1 `project.md` template and new §2.1.1 Language setup prose in `doc/sistema-sdd-pedro.md` (≤80 lines for §2.1.1 slice)
...
- [x] 2.1 Add `--chat-lang`, `--docs-lang`, `--code-lang` flags and `validate_locale()` allowlist (`en`, `pt-BR`) to `sdd-kit/install.sh`; default missing values to `en`
...
- [x] 2.2 Implement interactive TTY prompts (three numbered menus) when flags absent; log applied defaults
  - **Pattern:** `sdd-kit/install.sh`
...
- [x] 2.3 Substitute placeholders in `merge_agents_profile` / AGENTS generation; inject `## Language policy` into `openspec/project.md` via `<!-- SDD_LANGUAGE_POLICY_START -->` anchors
  - **Pattern:** `sdd-kit/install.sh`
...
- [x] 2.4 Smoke test in temp dir: `openspec init` stub or minimal `project.md` + `install.sh --profile DOCS_SPECS --chat-lang pt-BR --docs-lang en --code-lang en --repo <tmpdir>` — no `{{*}}` in AGENTS.md
  - **Pattern:** `sdd-kit/install.sh`
...
- [x] 3.1 Add language policy checks to `sdd-kit/verify.sh` (no `{{CHAT_LANG}}` leak; `project.md` has Language policy when present)
...
- [x] 3.2 Update `sdd-kit/templates/scripts/bootstrap-sdd.sh` post-install message to mention language flags / §2.1.1
...
- [x] 3.3 Regenerate MANIFEST checksums if any `sdd-kit/templates/` file changed
...
- [x] 4.1 Add §2.8 checklist item for Language policy in `project.md` + Communication in `AGENTS.md`
  - **Pattern:** `doc/sistema-sdd-pedro.md` §2.8
  - **Gate:** `grep -q 'Language policy' doc/sistema-sdd-pedro.md`

- [x] 4.2 Run OpenSpec validate and task-pattern verify
...
- [x] 5.1 Confirm proposal/design/specs state hub is grandfathered; no task modifies hub `AGENTS.md` or `openspec/project.md` Communication/Conventions
