...
- [x] 2.1 In the "Stack & companions" bullet, add GitHub links for GitNexus and Graphify next to the existing OpenSpec link (keep `agents.md` link unchanged).
  - **Pattern:** README.md
  - **Gate:** `grep -A5 'Stack & companions' README.md | grep -q 'github.com/abhigyanpatwari/GitNexus' && grep -A5 'Stack & companions' README.md | grep -q 'github.com/Graphify-Labs/graphify' && echo OK`

## 3. Verify

- [x] 3.1 Confirm no other README section was altered (docs-only diff, `README.md` is the only changed file).
