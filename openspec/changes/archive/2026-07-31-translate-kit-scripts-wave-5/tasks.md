...
- [x] 2.1 Rewrite `sdd-kit/install-ui-module.sh` Portuguese embedded infra UI-module table chrome and any residual Portuguese comments/operator strings → glossary-canonical English; keep `--detect` / `--dry-run` / `--apply` / `--yes` / stack detection / Impeccable install control flow intact
