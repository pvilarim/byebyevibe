...
- Dependencies: none (infra ✅ — `verify-i18n-wave.sh` already registered; skill paths not owned by active kit/infra/avaliacoes/design/`translate-skills-wave-1`/`translate-skills-wave-2`/`translate-skills-wave-3` changes or open translate PRs #78 / #84)
...
- **Non-goals:** other skills (`openspec-explore` / `openspec-archive-change`, review skills and `openspec-apply-change` already proposed); commands under `.cursor/commands/` / `.claude/commands/opsx/`; canonical guide; kit templates; hub `openspec/infra.md`; evaluations; design docs; dual-file `*.en.md` / `*-pt.md`; global G-DoD; path renames; changing propose workflow steps, AskUserQuestion flow, or §12.10 Gate/Pattern rules — language only
