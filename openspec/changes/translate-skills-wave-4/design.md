# Design — translate-skills-wave-4 (openspec-propose skill PT→EN)

## Context

- Layer-1 policy and wave budgets live under `sdd-docs-language` / `doc/i18n/*`.
- Active translate ownership on current base includes kit W2c/W2d (apply complete / in PR), hub `openspec/infra.md`, `correctness-review` / `simplify-review` / `openspec-apply-change` skills (waves 1–3), avaliacoes-wave-1, design-wave-1/2. None own `openspec-propose` paths.
...
- Other opsx skills (`openspec-explore`, `openspec-archive-change`) and skills already proposed (waves 1–3).
...
- `openspec/changes/translate-skills-wave-3/` — prior WSk propose pattern (apply skill)
...
| D — `openspec-archive-change` | Deferred — valid later WSk wave; propose is next after apply (wave-3) |
| E — `openspec-propose` as `translate-skills-wave-4` | **Chosen** — next deferred WSk residual after apply; within LOC; whole-file G-PT + G-MIRROR; disjoint from owned set |
...
**Rationale:** Same pattern as skills-wave-1 / skills-wave-2 / skills-wave-3 ADDED requirements.
...
- None blocking propose. Follow-up residual candidates after this: other opsx skills (`openspec-explore` / `openspec-archive-change`), opsx commands (≤4 files or paired mirrors), course WCu, `doc/design/001` split strategy, kit `sdd-kit/templates/doc/design/` mirrors (checksum-aware), guide G-PT strategy.
