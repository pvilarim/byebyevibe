...
- [ ] 1.1 Substituir H1 e abrir hero com ByeByeVibe + tagline + anti-boilerplate canónicos (design D1); manter CTA `sdd-kit/install.sh`
...
- [ ] 1.2 Adicionar glossário dual naming (ByeByeVibe = public name; `sdd-kit/` = payload) e actualizar blurb About no checklist manual do README
...
- [ ] 2.1 Actualizar título/intro de `sdd-kit/README.md` para ByeByeVibe; preservar comandos `sdd-kit/`
  - **Gate:** `grep -qi 'ByeByeVibe' sdd-kit/README.md && grep -q 'sdd-kit/install.sh' sdd-kit/README.md`
  - **Pattern:** `sdd-kit/README.md`

- [ ] 2.2 Actualizar `openspec/project.md` e ponteiros de discovery em `AGENTS.md` / guia § first-contact para o nome público (sem mudar paths)
...
- [ ] 3.1 Marcar P10 como adoptado (ByeByeVibe) em `doc/avaliacoes/2026-07-26-sdd-discovery-positioning.md`; checklist `[AÇÃO MANUAL]` com rename repo → `byebyevibe`, About, topics
...
- [ ] 3.2 Cosmético opcional: headers/echo topo `sdd-kit/install.sh`, `upgrade.sh`, `verify.sh` → `ByeByeVibe (sdd-kit)` sem alterar paths; se templates sob `sdd-kit/templates/` mudarem, correr `bash sdd-kit/gen-manifest-checksums.sh`
