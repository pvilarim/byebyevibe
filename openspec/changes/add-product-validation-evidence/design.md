...
**Non-Goals:** APP implementation here; a browser/testing framework; an always-on observer; kit distribution; CI or G4 rewrites; Astra adoption, installation, legacy migration, dashboard or model-routing changes. Human visual acceptance is recorded, not promoted to release approval.
...
2. **Proportional gates.** Behavior-changing tasks select relevant functional/visual/recovery scenarios. Documentation-only work uses semantic scenario walkthroughs; no browser requirement. Unrelated one-line edits need no benchmark dossier. Keep deterministic Gate commands and add a semantic evidence reference when applicable. Update existing hub propose/apply skills and actual duplicated command bodies, leaving kit templates for a later distribution change.
...
- Mirrors may drift → inventory actual hub copies at apply, update duplicated instructions, review semantic consistency. No kit payload is changed by this scope.
