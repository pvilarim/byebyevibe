...
- [x] 0.1 Executar piloto em worktree APP com Vitest ou pytest: C1 + GitNexus + Graphify + Probity; medir latência PreToolUse p95 (< 8s), falsos positivos tipo C (< 15%), R6 tipo B (100%), Cursor hooks
...
- [x] 0.2 Registar resultado do piloto em nota no change ou issue; se falhou → status "Adiado" em avaliação G2 e não prosseguir §6 MANIFEST bump
...
- [x] 1.1 Criar `sdd-kit/templates/install-probity-module.sh` com `--detect`, `--dry-run`, `--apply`, `--yes`, `--uninstall`, `--repo`; detectar vitest/jest/pytest; SKIP DOCS_SPECS sem test runner
  - **Pattern:** `sdd-kit/install-ui-module.sh`
  - **Gate:** `test -f sdd-kit/templates/install-probity-module.sh`

- [x] 1.2 Copiar/symlink para `sdd-kit/install-probity-module.sh` executável
  - **Pattern:** `sdd-kit/install-ui-module.sh`
  - **Gate:** `test -x sdd-kit/install-probity-module.sh`

- [x] 1.3 Criar `sdd-kit/templates/probity.config.ts` com `enforceTdd()` (addendum SDD R6), globs prod+test, exclusões doc/openspec/sdd-kit, `forbidCommandPattern(/rm\s+-rf/)`
  - **Pattern:** `openspec/changes/add-probity-tdd-module/design.md`
  - **Gate:** `test -f sdd-kit/templates/probity.config.ts && grep -q 'enforceTdd' sdd-kit/templates/probity.config.ts`

- [x] 1.4 Criar `sdd-kit/templates/doc/design/004-probity-module-install.md` (install, plugin, piloto, Cursor, rollback, lint opt-in)
...
- [x] 1.5 Actualizar `sdd-kit/README.md`: cenário Probity (G2), comando install, pin `@nizos/probity@1.10.0`
  - **Pattern:** `sdd-kit/README.md`
  - **Gate:** `grep -qi 'install-probity-module' sdd-kit/README.md`

- [x] 1.6 Adicionar entradas em `sdd-kit/MANIFEST.yaml` (`install-probity-module.sh`, `probity.config.ts` template, `004-probity-module-install.md`) com `profiles: [APP, HYBRID]`; correr `bash sdd-kit/gen-manifest-checksums.sh`
...
- [x] 1.7 Adicionar check Probity em `sdd-kit/verify.sh` (report-only se módulo não instalado)
  - **Pattern:** `sdd-kit/verify.sh`
  - **Gate:** `grep -q 'probity' sdd-kit/verify.sh`

## 2. openspec/infra.md (R1)

- [x] 2.1 Adicionar secção "Probity Module" em `openspec/infra.md` e template: `@nizos/probity@1.10.0`, estado SKIP (hub) ou ✅, `test -f probity.config.ts`
  - **Pattern:** `openspec/infra.md`
  - **Gate:** `grep -q 'probity' openspec/infra.md`

- [x] 2.2 Espelhar secção em `sdd-kit/templates/openspec/infra.md` (se template existir) ou documentar merge em install script
...
- [x] 3.1 Adicionar ≤10 linhas em Integrações Probity (quando activo, matriz A–E, comando install); actualizar pipeline reviews: `testes (R6/Probity enforceTdd)` → correctness-review → …
  - **Pattern:** `AGENTS.md`
  - **Gate:** `grep -q 'Probity' AGENTS.md && grep -q 'enforceTdd' AGENTS.md`

- [x] 3.2 Actualizar `sdd-kit/templates/AGENTS.core.md` com mesmas linhas
...
- [x] 4.1 Se necessário: criar `.claude/skills/probity-guard/SKILL.md` + espelho `.cursor/skills/probity-guard/SKILL.md` (troubleshooting enforceTdd, override, desinstalar)
...
- [x] 5.1 Adicionar §2.16 "Módulo Probity (G2)" em `doc/sistema-sdd-pedro.md`: install plugin, piloto, Cursor third-party hooks, desligar (globs/uninstall), troubleshooting, rollback; renumerar §2.14 github-mcp se necessário
  - **Pattern:** `doc/sistema-sdd-pedro.md`
  - **Gate:** `grep -q 'Probity' doc/sistema-sdd-pedro.md && grep -q '2.16' doc/sistema-sdd-pedro.md`

- [x] 5.2 Actualizar referências "TDD Guard" → "Probity" na pipeline §2.14 correctness-review
  - **Pattern:** `doc/sistema-sdd-pedro.md`
...
- [x] 6.1 `doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md`: G2 → Probity; nota supersessão TDD Guard; piloto; link change `add-probity-tdd-module`
  - **Pattern:** `doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md`
...
- [x] 6.2 `doc/avaliacoes/README.md`: índice linha 2026-07-25 → "Probity (G2)" em vez de TDD Guard
  - **Pattern:** `doc/avaliacoes/README.md`
  - **Gate:** `grep -q 'Probity' doc/avaliacoes/README.md`

- [x] 6.3 `openspec/changes/explore-oss-coverage-gaps/research.md`: G2 candidato Probity; matriz; ordem impl. #5; riscos; links; nota histórica TDD Guard
...
- [x] 6.4 `openspec/changes/explore-oss-coverage-gaps/metodologia-insercao.md`: todas refs "TDD Guard" → "Probity (G2)"; toggle → globs/desligar módulo
...
- [x] 6.5 `openspec/project.md`: cross-reference G2/Probity se aplicável (só se mencionar TDD — verificar; adicionar linha testes+Probity opcional)
  - **Pattern:** `openspec/project.md`
  - **Gate:** `grep -q 'Probity' openspec/project.md || ! grep -q 'TDD' openspec/project.md`

- [x] 6.6 Actualizar skills correctness-review: `.claude/skills/correctness-review/SKILL.md` e `.cursor/skills/correctness-review/SKILL.md` — pipeline "R6/Probity" em vez de "R6/TDD Guard"
  - **Pattern:** `.claude/skills/correctness-review/SKILL.md`
  - **Gate:** `grep -q 'Probity' .claude/skills/correctness-review/SKILL.md`

## 7. Specs (promover após apply)

- [x] 7.1 Promover `openspec/changes/add-probity-tdd-module/specs/sdd-probity-module/spec.md` → `openspec/specs/sdd-probity-module/spec.md`
  - **Pattern:** `openspec/specs/sdd-ui-module/spec.md`
  - **Gate:** `test -f openspec/specs/sdd-probity-module/spec.md`

- [x] 7.2 Aplicar delta `sdd-correctness-review` em `openspec/specs/sdd-correctness-review/spec.md` (pipeline R6/Probity)
  - **Pattern:** `openspec/specs/sdd-correctness-review/spec.md`
  - **Gate:** `grep -q 'Probity' openspec/specs/sdd-correctness-review/spec.md`

## 8. Avaliação (R5)

- [x] 8.1 Actualizar G2 em `doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md` para "Adoptado" após piloto + archive (pendente até lá: "Adoptado — change add-probity-tdd-module, piloto pendente")
  - **Pattern:** `doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md`
  - **Gate:** `grep -q 'add-probity-tdd-module' doc/avaliacoes/2026-07-25-oss-coverage-gaps-tooling.md`

## 9. Validação

- [x] 9.1 Correr `scripts/verify-task-patterns.sh` sobre este `tasks.md`
