...
- [ ] 1.2 Mirror to `.cursor/skills/sdd-tooling-guidance/SKILL.md` — same body except the documented asymmetry: replace harness-tool references with "suggest + point to `doc/tooling-install.md`" (no Claude Code tool names presented as available)
...
- [ ] 1.3 Copy both live skills into `sdd-kit/templates/.claude/skills/sdd-tooling-guidance/SKILL.md` and `sdd-kit/templates/.cursor/skills/sdd-tooling-guidance/SKILL.md`
...
- [ ] 2.1 Extend R10 in `AGENTS.md` with one sentence stating the resolution order for external-tool actions (override → CLI → MCP → suggest → manual)
  - **Pattern:** `AGENTS.md`
...
- [ ] 2.2 Add compact "Tooling cascade (CLI → MCP → manual)" section (~10 lines: cascade skeleton, second-manual-fall trigger, shared cap, pointer to `sdd-tooling-guidance`) to `.claude/commands/opsx/apply.md` and `.claude/skills/openspec-apply-change/SKILL.md`
...
- [ ] 2.3 Same section on `.cursor/commands/opsx-apply.md` and `.cursor/skills/openspec-apply-change/SKILL.md`
...
- [ ] 4.1 Extend `scripts/verify-infra.sh` with an advisory "Tooling gap-check" report: presence/absence of `.mcp.json` and `.cursor/mcp.json`, `PATH` availability of CLIs listed in `openspec/infra.md`, `.env.example` key names (commented-out key reported as "considered and declined"), suppression of rows whose manifest status is `declined`; gaps NEVER cause a non-zero exit; existing marker ownership untouched (Preflight section remains `preflight-sdd.sh`'s)
...
- [ ] 4.2 Document the `declined` status value in `openspec/infra.md` (Agent rule section: `declined` = durable refusal, do not re-suggest; cascade rungs still apply) and sync `sdd-kit/templates/scripts/verify-infra.sh` with the live script
...
- [ ] 5.2 Sync `sdd-kit/templates/doc/sdd-operator-day1.md` with the live doc
  - **Gate:** `diff -q doc/sdd-operator-day1.md sdd-kit/templates/doc/sdd-operator-day1.md`

- [ ] 5.3 Update `openspec-help` skill (live `.claude` + `.cursor` and both kit templates): section table gains the §8 Tooling row, `§0–§8` → `§0–§9`, handoff ref `§8` → `§9`, one "Keep framing honest" bullet on tooling (cascade; suggest-only; CLI default)
...
- [ ] 5.4 Update `/opsx:help` command mirrors (`.claude/commands/opsx/help.md`, `.cursor/commands/opsx-help.md`, and both kit template copies) `§0–§8` → `§0–§9`
...
- [ ] 8.1 Add MANIFEST entries for the new templates (two skill mirrors, `doc/tooling-install.md`; profiles APP/DOCS_SPECS/HYBRID, merge COPY, documentary `gate:` fields)
...
- [ ] 8.2 Regenerate checksums for new and modified templates
  - **Gate:** `bash sdd-kit/gen-manifest-checksums.sh && bash sdd-kit/gen-manifest-checksums.sh --check`

- [ ] 8.3 Create `doc/avaliacoes/2026-08-01-tooling-guidance.md` from the template (Decision: Adopted; cites explore research D1–D12) and add its row to `doc/avaliacoes/README.md`
...
- [ ] 9.1 OpenSpec strict validation passes for this change
  - **Gate:** `npx -y @fission-ai/openspec@1.3.1 validate add-tooling-guidance --strict`

- [ ] 9.2 Task patterns verify clean (DOCS_SPECS boundary)
