...
- [ ] 1.1 Remover o primeiro bloco `python3 - <<'PY'` (linhas 130–160) que imprime TSV bruto para stdout sem consumidor; manter apenas o segundo bloco (dentro do `while IFS=$'\t'` process substitution)
...
- [ ] 2.1 Linha 5 de `doc/sistema-sdd-pedro.md`: `v1.3.2` → `v1.4.0`
  - **Pattern:** `doc/sistema-sdd-pedro.md`
  - **Gate:** `grep -q 'v1.4.0' doc/sistema-sdd-pedro.md`

- [ ] 2.2 Linha 149 (prompt §2.0) de `doc/sistema-sdd-pedro.md`: `v1.3.0` → `v1.4.0`
  - **Pattern:** `doc/sistema-sdd-pedro.md`
  - **Gate:** `grep -c 'v1.4.0' doc/sistema-sdd-pedro.md | grep -qv '^0$'`

## 3. F-NORM-1 — Promover spec sdd-ci-gates + corrigir gate task 6.2

- [ ] 3.1 Criar `openspec/specs/sdd-ci-gates/spec.md` (cópia de `openspec/changes/add-sdd-ci-gates-workflow/specs/sdd-ci-gates/spec.md`)
  - **Pattern:** `openspec/specs/sdd-install-kit/spec.md`
  - **Gate:** `test -f openspec/specs/sdd-ci-gates/spec.md`

- [ ] 3.2 Corrigir gate da task 6.2 em `openspec/changes/add-sdd-ci-gates-workflow/tasks.md`: `test -f openspec/changes/add-sdd-ci-gates-workflow/specs/sdd-ci-gates/spec.md` → `test -f openspec/specs/sdd-ci-gates/spec.md`
...
- [ ] 4.1 Tabela "Contexto sob demanda": adicionar linha `| Gates de CI (sdd-gates, operação) | \`doc/sistema-sdd-pedro.md\` §2.12 · \`.github/workflows/sdd-gates.yml\` |`
  - **Pattern:** `sdd-kit/templates/AGENTS.core.md`
  - **Gate:** `grep -q 'Gates de CI' sdd-kit/templates/AGENTS.core.md`

- [ ] 4.2 Secção "Integrações": adicionar bloco `**CI Gates (sdd-gates)**` com referência ao workflow e §2.12
  - **Pattern:** `sdd-kit/templates/AGENTS.core.md`
  - **Gate:** `grep -q 'CI Gates' sdd-kit/templates/AGENTS.core.md`

- [ ] 4.3 Tabela "Commands" (bloco `SDD_KIT_COMMANDS`): não aplicável neste template (placeholder) — adicionar nota no bloco de Integrações
  - **Pattern:** `sdd-kit/templates/AGENTS.core.md`
  - **Gate:** `grep -q 'sdd-gates' sdd-kit/templates/AGENTS.core.md`

## 5. F-NORM-3 — Actualizar referências de versão em AGENTS.md

- [ ] 5.1 `AGENTS.md` linha com `(v1.3)`: `(v1.3)` → `(v1.4.0)`
  - **Pattern:** `AGENTS.md`
  - **Gate:** `! grep -q '(v1.3)' AGENTS.md`

- [ ] 5.2 `AGENTS.md` linha com `(v1.3.1)`: `(v1.3.1)` → `(v1.4.0)`
  - **Pattern:** `AGENTS.md`
  - **Gate:** `! grep -q '(v1.3.1)' AGENTS.md`

## 6. F-NORM-6 — Actualizar timestamp openspec/infra.md

- [ ] 6.1 Linha 2 de `openspec/infra.md`: `2026-06-17` → `2026-07-25`
