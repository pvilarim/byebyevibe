...
- **`sdd-session-register.sh`** — lock adquirido **antes** de escrever o ficheiro de sessão; `trap 'sdd_session_stop_lock_holder' EXIT` cobre o intervalo entre adquirir o lock e completar o registo — se `write_json` ou o ponteiro falharem depois do lock adquirido, o lock é libertado em vez de ficar órfão (gap encontrado por revisão do Cursor Bugbot, corrigido antes do merge). Emite `SESSION_ID=<uuid>` parseável em stdout (log humano movido para stderr).
...
- **`sdd-session-heartbeat.sh` / `sdd-session-release.sh`** — aceitam `--session-id` explícito, com aviso ao cair no fallback `current-session.id` quando há mais de uma sessão activa na worktree. `release.sh` liberta o lock holder da worktree quando **nenhuma outra sessão apply** continua registada (novo helper `sdd_session_has_other_apply_session`) — cobre tanto libertar a própria apply como um lock verdadeiramente órfão (sem session file), sem nunca afectar uma sessão apply diferente ainda activa (gap encontrado por revisão do Cursor Bugbot, corrigido antes do merge).
...
- Testado end-to-end (incluindo injecção de falhas): self-skip sob ponteiro sobrescrito, conflito real entre sessões, `register.sh` recusa segunda apply, `release.sh` selectivo não afecta lock de outra sessão, lock órfão libertado mesmo sem session file/ponteiro, falha após lock adquirido não deixa processo órfão.
