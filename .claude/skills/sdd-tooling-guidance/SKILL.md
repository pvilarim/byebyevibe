...
**Surface note (asymmetry):** this is the Claude Code mirror. It MAY reference native harness tools (`SearchMcpRegistry`, `ListConnectors`) to help the operator locate an MCP or connector when they ask. The Cursor mirror intentionally omits those tool names and degrades to "suggest + point to `doc/tooling-install.md`" — no parity assumed.
...
- On Claude Code you MAY use `SearchMcpRegistry` / `ListConnectors` to help locate an official MCP server or connector — locating is not installing; the operator runs every install/configure step themselves.
